...
class Balance extends Driver
{
    ...
    public function transferBalance(array $data = [], array $input = []): array
    {
        $data = [
            ...
            'balance' => $this->revertAmount($data['balance']), // 分
    ...
    {
        $params1 = [
            'mchId' => $input['merchant_id'],
            'queryTime' => $this->getTimestamp(),
        ];

        // 串接 sign_key
        $sign_key = $input['merchant_key'] . $input['body_params']['customer_service_key'];
