...
class Driver extends AbstractDriver implements DriverPaymentInterface, DriverWithdrawInterface
{
    ...
    {
        // 三方支付状态: 0=待处理, 1=处理中, 2=成功, 3=取消 , 4=失败
    ...
    {
        // 三方支付状态: 0=预约中, 1=待处理, 2=处理中, 3=成功, 4=取消, 5=失败, 6=审核中
    ...
    public function responsePlatform(string $code = ''): ResponseInterface
    {
        // 集成网关返回
        if ('00' != $code) {
            return response()->json(['error_code' => $this->notifyFailText]);
        }

        return response()->json(['error_code' => $this->notifySuccessText]);
    }
