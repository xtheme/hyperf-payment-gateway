...
class OrderCreate extends Driver
{
    ...
    public function request(RequestInterface $request): ResponseInterface
    {
        ...
        // 更新訂單
        $update = [
            'trade_no' => $response['data']['serialOrderNo'], // 三方交易号,建單無三方訂單編號
        ...
        $data = [
            ...
            'link' => $response['data']['url']['payUrl'], // 支付网址
            'trade_no' => $response['data']['serialOrderNo'], // 三方交易号
            'payee_name' => $response['data']['accountName'] ?? '', // 收款人姓名
            'payee_bank_name' => $response['data']['bankName'] ?? '', // 收款人开户行
            'payee_bank_branch_name' => $response['data']['branchName'] ?? '', // 收款行分/支行
            'payee_bank_account' => $response['data']['accountNumber'] ?? '', // 收款人账号
    ...
    protected function prepareOrder(string $orderNo, array $data = []): array
    {
        ...
        // 準備三方所需參數
        $params = [
            'mchKey' => $data['merchant_id'],
            'product' => $data['payment_channel'], // 通道代碼
            'mchOrderNo' => $data['order_id'],
            'amount' => (int) $this->convertAmount($data['amount']), // 分
            'nonce' => $rand,
