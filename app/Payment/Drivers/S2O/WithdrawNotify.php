...
class WithdrawNotify extends WithdrawQuery
{
    ...
    public function request(RequestInterface $request): ResponseInterface
    {
        ...
        // 二次校驗
        $orderInfo = $this->doubleCheck($callback, $orderNo, $order);

        // 更新订单
        $update = [
            'status' => $this->transformWithdrawStatus($orderInfo['order_status']),
        ];
        ...
        // 返回数据给集成网关更新
        $params = $this->transferOrderInfo($orderInfo, $order);
    ...
    private function doubleCheck(array $callback, string $orderNo, array $order)
    {
        ...
        if ('success' == $callback['status']) {
            if ($response['order_info']['order_status'] != $callback['status']) {
                throw new ApiException('訂單狀態確認失敗 ' . $orderNo);
            }
        }

        // 回調的status只有成功失敗, 改用查詢的status
