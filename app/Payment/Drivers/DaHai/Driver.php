...
class Driver extends AbstractDriver implements DriverPaymentInterface, DriverWithdrawInterface
{
    ...
    protected string $signField = 'EncryptValue';
    ...
    {
        // 三方支付状态: 1:待支付 2:成功 3:失败(付款金额错误、实名制审核失败，未付款则不回调)
    ...
    {
        // 三方支付状态: 1:待支付 2:成功 3:取消(驳回) 4:待审核
    ...
    protected function getSignature(array $data, string $signatureKey): string
    {
        ...
        // 4. $tempStr 拼接密鑰
        $tempStr .= '&HashKey=' . $signatureKey;

        // 5. 字串组好后直接整串转为英文小写
        $tempStr = strtolower($tempStr);

        // 6. sign = $tempStr 進行 sha256 後轉為大寫
        return strtoupper(hash('sha256', $tempStr));
    }
}
