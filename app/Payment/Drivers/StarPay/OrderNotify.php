...
class OrderNotify extends OrderQuery
{
    ...
    public function request(RequestInterface $request): ResponseInterface
    {
        ...
        $tempStr = (string) $callback['amount'] . $callback['channelCode'] . $callback['email'] . $callback['merchantCode'] . $callback['orderNumber'] . $callback['phone'] . $callback['userName'] . $order['merchant_key'];

        // 验证签名
        if ($callback[$this->signField] !== md5($tempStr)) {
