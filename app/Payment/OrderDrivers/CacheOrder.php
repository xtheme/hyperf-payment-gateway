...
class CacheOrder extends AbstractOrder implements OrderDriverInterface
{
    // 允許更新的欄位
    ...
    public function createOrder(string $orderNo, array $data, string $type = 'order'): void
    {
        ...
        Cache::put($cacheKey, $order, 86400 * 90);
    }
    ...
    public function getOrder(string $orderNo, string $type = 'order')
    {
        ...
        if (!$order) {
            throw new ApiException('订单记录不存在或已过期 ' . $orderNo . ', cacheKey: ' . $cacheKey);
        }
    ...
    public function updateOrder(string $orderNo, array $data, string $type = 'order'): void
    {
        ...
        $merge = array_merge($order, $data);

        $merge['updated_at'] = $this->getServerDateTime(); // 更新時間

        $log = sprintf('%s: %s updateOrder', $type, $orderNo);
        $this->logger->info($log, $data);

        if ('order' == $type && '2' == $merge['status']) {
            $merge['lock'] = 1; // 回調成功後鎖定訂單, 防止重複回調
        }

        if ('withdraw' == $type && '3' == $merge['status']) {
            $merge['lock'] = 1; // 回調成功後鎖定訂單, 防止重複回調
        }

        Cache::put($cacheKey, $merge, 86400 * 90);
    }
}
